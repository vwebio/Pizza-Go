...
const cartTotalPrice = document.querySelectorAll('.modal-cart__price');
const nextBtn = document.querySelector('.modal-cart__continue');
const prevBtn = document.querySelector('.modal-cart__prev');
const cartBlock = document.querySelector('.modal-cart__block');
const cartDelivery = document.querySelector('.modal-cart__delivery');
const submitBtn = document.querySelector('.modal-cart__send-order');
const cartForm = document.querySelector('.modal-cart__form');

// Функция для рендеринга списка товаров в корзине
  ...
  if (cartControl.cartData.length) {
    ...
    const cardsData = await Promise.all(
      cartControl.cartData.map(
        async item => await getData(`products/${item.id}`)
      )
    );
    ...
    const cardsCart = cardsData.map((data, i) => {
      ...
      cardCart.innerHTML = `
        ...
        <div class="modal-cart__content">
          <h3 class="modal-cart__title-pizza">${toUpperCaseFirstLetter(data.name.ru)}</h3>
    ...
    cartList.append(...cardsCart);

  } else {
    // Если в корзине нет товаров
    cartList.innerHTML = '<li>В корзине ничего нет</li>';
    nextBtn.disabled = true;
  }

  // Обновляем общую стоимость товаров в корзине
  cartTotalPrice.forEach(item => item.textContent = `${totalPrice} ₽`);
};

// Функция для удаления товара из корзины
const delCartItem = ({target}) => {
  const deleteBtn = target.closest('.modal-cart__delete-button');

  if (deleteBtn) {
    const productId = deleteBtn.dataset.id;
    cartControl.removeCart(productId);
    renderCartList();
  }
};

// Функция для перехода к следующему блоку
const nextBlock = () => {
  cartBlock.style.display = 'none';
  cartDelivery.style.display = 'block';
};

// Функция для возврата к предыдущему блоку
const prevBlock = () => {
...
// Экспортируем функцию для управления модальным окном корзины
